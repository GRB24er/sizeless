...
export default function Loading() {
  ...
  useEffect(() => {
    const statuses = [
      "Initializing secure connection...", "Authenticating session...",
      "Loading shipment data...", "Syncing tracking updates...",
      "Connecting vault services...", "Preparing dashboard...",
    ];
    let currentStatus = 0;
    const statusInterval = setInterval(() => { currentStatus = (currentStatus + 1) % statuses.length; setStatusText(statuses[currentStatus]); }, 1500);
    const progressInterval = setInterval(() => { setProgress((prev) => (prev >= 90 ? prev : prev + Math.random() * 15)); }, 300);
