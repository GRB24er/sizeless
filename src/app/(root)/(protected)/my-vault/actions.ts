...
export async function requestVaultRelease(depositId: string) {
  ...
  if (user?.email) {
    await sendVaultEmail(user.email, user.name, `Release Request — ${deposit.depositNumber} | Aegis Cargo`,
      ...
      <div style="text-align:center;margin:24px 0;"><a href="https://www.aegiscargo.org/vault" style="display:inline-block;background:#D4A853;color:#0A1628;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:600;font-size:14px;">View Vault Dashboard</a></div>`
    );
  }
