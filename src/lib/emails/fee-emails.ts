...
export async function sendFeeInvoiceEmail(data: FeeEmailData): Promise<boolean> {
  ...
  try {
    await transporter.sendMail({
      ...
      subject: `💰 Payment Required — ${feeLabel} for ${data.trackingNumber} | AramexLogistics`,
      html: `
<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;">
  ...
  <div style="background:linear-gradient(135deg,#0A1628 0%,#0D1F35 100%);padding:32px;text-align:center;">
    ...
    <p style="color:#d4a853;font-size:14px;margin:8px 0 0;">Invoice ${data.invoiceNumber}</p>
  </div>
  ...
  <div style="padding:32px;">
    ...
    <!-- Amount Banner -->
    <div style="background:linear-gradient(135deg,#0A1628,#1a2d4a);border-radius:12px;padding:24px;text-align:center;margin:24px 0;">
      <p style="color:#9ca3af;font-size:12px;text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;">Amount Due</p>
      <p style="color:#d4a853;font-size:36px;font-weight:700;margin:0;">${formattedAmount}</p>
    ...
    <div style="background:#f9fafb;border-radius:12px;padding:20px;margin:24px 0;">
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr>
          ...
          <td style="padding:10px 0;color:#059669;font-size:14px;font-weight:600;text-align:right;border-bottom:1px solid #e5e7eb;">${data.trackingNumber}</td>
        </tr>
        <tr>
    ...
    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:12px;padding:16px;margin:24px 0;">
      <p style="color:#92400e;font-size:13px;margin:0;line-height:1.5;">
        <strong>⚠️ Important:</strong> Your shipment will remain on hold until this payment is confirmed. Please contact us at <a href="mailto:admin@aramexlogistics.org" style="color:#059669;">admin@aramexlogistics.org</a> or call <strong>+44 020 1412 251</strong> to arrange payment.
      </p>
    </div>

    <!-- CTA -->
    <div style="text-align:center;margin:32px 0;">
      <a href="https://www.aramexlogistics.org/track" style="display:inline-block;background:#059669;color:#fff;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:600;font-size:14px;">Track Your Shipment</a>
    </div>
  </div>

  <!-- Footer -->
  <div style="background:#f9fafb;padding:24px 32px;border-top:1px solid #e5e7eb;text-align:center;">
    <p style="color:#9ca3af;font-size:12px;margin:0;">© ${new Date().getFullYear()} AramexLogistics Ltd | Registered in England & Wales</p>
    <p style="color:#9ca3af;font-size:12px;margin:4px 0 0;">admin@aramexlogistics.org | +44 020 1412 251</p>
  </div>
</div>`,
    });
    return true;
...
export async function sendFeeReceiptEmail(
  ...
  try {
    await transporter.sendMail({
      from: FROM_EMAIL,
      to: data.recipientEmail,
      subject: `✅ Payment Confirmed — ${feeLabel} for ${data.trackingNumber} | AramexLogistics`,
      attachments: [
        {
...
<div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;">
  ...
  <div style="background:linear-gradient(135deg,#0A1628 0%,#0D1F35 100%);padding:32px;text-align:center;">
    ...
    <p style="color:#059669;font-size:14px;margin:8px 0 0;">Receipt ${data.invoiceNumber}</p>
  </div>
  ...
  <div style="padding:32px;">
    ...
    <!-- Success Banner -->
    <div style="background:#f0fdf4;border:1px solid #a7f3d0;border-radius:12px;padding:24px;text-align:center;margin:24px 0;">
      <p style="color:#065f46;font-size:14px;text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;">Payment Received</p>
      <p style="color:#059669;font-size:36px;font-weight:700;margin:0;">${formattedAmount}</p>
      <p style="color:#065f46;font-size:13px;margin:8px 0 0;">Paid on ${paidDate}</p>
    </div>
    ...
    <div style="background:#f9fafb;border-radius:12px;padding:20px;margin:24px 0;">
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr>
          ...
          <td style="padding:10px 0;text-align:right;"><span style="display:inline-block;background:#059669;color:#fff;padding:4px 12px;border-radius:50px;font-size:12px;font-weight:600;">PAID</span></td>
        </tr>
      </table>
    </div>
