...
const FROM = "AramexLogistics Vault <admin@aramexlogistics.org>";
const SITE = "https://www.aramexlogistics.org";
...
<div style="max-width:600px;margin:0 auto;background:#ffffff;">
  ...
  <div style="background:linear-gradient(135deg,#0A1628 0%,#0D1F35 100%);padding:32px;text-align:center;">
    <img src="${SITE}/images/logo.png" alt="AramexLogistics" style="height:48px;margin-bottom:16px;" />
    <h1 style="color:#D4A853;font-size:20px;margin:0;font-weight:600;">Vault Custody Service</h1>
    <p style="color:#94a3b8;font-size:12px;margin:8px 0 0 0;">LBMA Approved • Fully Insured • 24/7 Security</p>
  </div>
  
  <!-- Gold accent bar -->
  <div style="height:3px;background:linear-gradient(90deg,#D4A853,#C09740,#D4A853);"></div>
  ...
  <div style="padding:32px;">
    ...
    ${ctaUrl ? `<div style="text-align:center;margin:28px 0;">
      <a href="${ctaUrl}" style="display:inline-block;background:linear-gradient(135deg,#D4A853,#C09740);color:#0A1628;text-decoration:none;padding:14px 36px;border-radius:8px;font-weight:700;font-size:14px;">${ctaLabel || "View in Dashboard"}</a>
    </div>` : ""}
  </div>
  
  <!-- Footer -->
  <div style="background:#f9fafb;padding:24px 32px;border-top:1px solid #e5e7eb;">
    <p style="color:#9ca3af;font-size:11px;margin:0 0 4px 0;text-align:center;">AramexLogistics Vault Services Ltd. | admin@aramexlogistics.org | +44 020 1412 251</p>
...
export async function notifyKYCApproved(email: string, name: string, depositNumber: string) {
  ...
  await sendVault(email, `KYC Approved — Deposit ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyKYCRejected(email: string, name: string, depositNumber: string, reason?: string) {
  ...
  await sendVault(email, `KYC Update — Deposit ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyIntakeScheduled(
  ...
  const html = vaultEmailTemplate(
    ...
    ${detailTable(
      ...
      detailRow("Facility", "AramexLogistics Vault, London")
    )}
  ...
  await sendVault(email, `Intake Appointment — ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyIntakeComplete(email: string, name: string, depositNumber: string) {
  ...
  await sendVault(email, `Assets Received — ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyAssayComplete(
  ...
  await sendVault(email, `Assay Results — ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyPlacedInStorage(
  ...
  const html = vaultEmailTemplate(
    ...
    <p style="color:#374151;font-size:14px;line-height:1.7;">Your custody reference number is <strong style="font-family:monospace;color:#0A1628;font-size:16px;">${custodyRef}</strong>. Keep this safe — you'll need it for all future transactions.</p>
  ...
  await sendVault(email, `In Secure Storage — ${custodyRef} | AramexLogistics Vault`, html);
}
...
export async function notifyInsuranceActivated(
  ...
  const html = vaultEmailTemplate(
    ...
    <p style="color:#6b7280;font-size:13px;">Coverage is continuous and renews automatically. Claims must be reported within 48 hours. Contact claims@aramexlogistics.org for any questions.</p>`,
    `${SITE}/my-vault`, "View Policy Details"
  );
  await sendVault(email, `Insurance Active — ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyReleaseRequested(email: string, name: string, depositNumber: string) {
  ...
  await sendVault(email, `Release Request — ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyReleaseApproved(email: string, name: string, depositNumber: string) {
  const html = vaultEmailTemplate(
    ...
    <p style="color:#374151;font-size:14px;line-height:1.7;">Please contact our vault team at <strong>admin@aramexlogistics.org</strong> to arrange collection or delivery. You will need to present:</p>
  ...
  await sendVault(email, `Release Approved — ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyReleased(email: string, name: string, depositNumber: string) {
  const html = vaultEmailTemplate(
    ...
    <p style="color:#374151;font-size:14px;line-height:1.7;">Thank you for using AramexLogistics Vault Services. We hope to serve you again in the future.</p>
  ...
  await sendVault(email, `Assets Released — ${depositNumber} | AramexLogistics Vault`, html);
}
...
export async function notifyDocumentsIssued(email: string, name: string, depositNumber: string, custodyRef: string) {
  const html = vaultEmailTemplate(
    ...
    <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border:2px solid #D4A853;border-radius:12px;padding:24px;margin:20px 0;text-align:center;">
      <p style="color:#92400e;font-size:11px;text-transform:uppercase;letter-spacing:2px;margin:0 0 8px 0;">Custody Reference</p>
      <p style="color:#0A1628;font-size:24px;font-weight:700;font-family:monospace;margin:0;">${custodyRef}</p>
    </div>
  ...
  await sendVault(email, `Documents Issued — ${custodyRef} | AramexLogistics Vault`, html);
}
